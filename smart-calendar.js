...
function canonicalContent(item){
 const meta=(window.META||{})[item.id]||{};
 return `content:${item.media||item.kind||''}:${meta.tmdbId||meta.id||item.query||item.title}:${item.year||''}`;
}
  ...
  if(all.length){
   ...
   for(const {season,ep} of all){
    ...
    if(map.has(canonical)){
     const kept=map.get(canonical);
     kept.done=kept.done||unit.done;
     duplicates.push(unit);
    }else map.set(canonical,unit);
   }
  }else{
...
function metrics(){
 ...
 const start=today();
 const target=new Date(((window.S?.target)||'2026-12-17')+'T12:00:00');
...
function baseline(m){
 ...
 if(!b||b.sig!==sig){
  b={sig,start:m.start.toISOString().slice(0,10),baseWatched:m.watched,total:m.total,initialPace:m.pace};
  localStorage.setItem(PLANKEY,JSON.stringify(b));
 }
 return b;
}
...
function rhythm(m){
 ...
 const delta=m.watched-expected;
 const dayDelta=ideal?delta/ideal:0;
 if(dayDelta>=.75)return{class:'ahead',icon:'🟢',title:`${Math.floor(dayDelta)} dia${Math.floor(dayDelta)===1?'':'s'} adiantado`,detail:`Sua média atual é ${fmt(m.pace)} por dia.`};
 if(dayDelta<=-.75)return{class:'late',icon:'🔴',title:`${Math.abs(Math.ceil(dayDelta))} dia${Math.abs(Math.ceil(dayDelta))===1?'':'s'} atrasado`,detail:`Para chegar a tempo, a média atual é ${fmt(m.pace)} por dia.`};
 return{class:'ok',icon:'🟡',title:'No cronograma',detail:`Mantenha em média ${fmt(m.pace)} por dia até a data-alvo.`};
}

function buildPlan(m){
 const days=[];
 for(let i=0;i<m.daysLeft;i++){
  const d=new Date(m.start);d.setDate(d.getDate()+i);
  days.push({date:d,minutes:0,items:[]});
 }
 let di=0;
 for(const u of m.remaining){
  if(di>=days.length)di=days.length-1;
  let d=days[di];
  if(d.items.length&&d.minutes+u.minutes>Math.max(1,m.pace)&&di<days.length-1)d=days[++di];
  d.items.push(u);d.minutes+=u.minutes;
 }
...
function hideLegacyStatus(){
 ...
 if(!dashboard)return;
 for(const el of [...dashboard.children]){
  if(el.id==='smartRhythmCard'||el.classList.contains('metrics')||el.classList.contains('block'))continue;
  const text=(el.textContent||'').toLowerCase();
  if((text.includes('você está atrasado')||text.includes('você está adiantado'))&&text.includes('ritmo ideal'))el.style.display='none';
 }
}
...
function updateProgress(m){
 ...
 if(ring)ring.style.background=`conic-gradient(var(--theme-accent-2,#65d0ff) ${pct*3.6}deg,#2b303b 0deg)`;
}
...
function renderSummary(){
 ...
 if(watched)watched.textContent=fmt(m.watched);
 if(remaining)remaining.textContent=fmt(m.left);
 if(pace)pace.textContent=`${fmt(m.pace)}/dia`;

 let card=document.querySelector('#smartRhythmCard');
 if(!card){
  card=document.createElement('section');
  card.id='smartRhythmCard';
  card.className='smartRhythmCard';
  document.querySelector('#dashboard .metrics')?.insertAdjacentElement('afterend',card);
 }
 if(card)card.innerHTML=`<div class="smartRhythmTop"><span>${r.icon}</span><div><small>RITMO ADAPTATIVO</small><h3>${r.title}</h3><p>${r.detail} Ao marcar episódios ou filmes, o tempo restante diminui e a meta diária é recalculada. Ao passar um dia sem progresso, há um dia a menos e a média pode subir.</p></div></div><button class="auditToggle" type="button">Detalhes do cálculo</button><div class="auditPanel" hidden><div><b>${fmt(m.total)}</b><span>tempo total auditado</span></div><div><b>${fmt(m.watched)}</b><span>tempo assistido</span></div><div><b>${fmt(m.left)}</b><span>tempo restante</span></div><div><b>${m.daysLeft}</b><span>dias restantes</span></div><div><b>${m.episodesLeft}</b><span>episódios pendentes</span></div><div><b>${m.duplicates.length}</b><span>duplicidades ignoradas</span></div></div>`;
 const btn=card?.querySelector('.auditToggle');
 if(btn)btn.onclick=()=>{const p=card.querySelector('.auditPanel');p.hidden=!p.hidden};
}
...
loadCatalog().then(()=>{
 ...
 document.addEventListener('click',e=>{
  if(e.target.closest('.episodeCardV5,[data-season-all],#markAllEpisodesV5,[data-v7ep],[data-done],#toggleDone,#v7Done,#saveSettings'))scheduleRefresh(180);
 },true);
