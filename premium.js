...
function tone(type='tap'){
 if(!P.sound)return;try{const c=audio(),now=c.currentTime,g=c.createGain();g.connect(c.destination);g.gain.setValueAtTime(.0001,now);g.gain.exponentialRampToValueAtTime(P.volume,now+.012);g.gain.exponentialRampToValueAtTime(.0001,now+.22);
 const notes={tap:[420,.08],nav:[300,.07],done:[620,.14],season:[440,.28],achievement:[523,.48],open:[260,.12]}[type]||[360,.1];
 const o=c.createOscillator();o.type=type==='achievement'?'triangle':'sine';o.frequency.setValueAtTime(notes[0],now);if(type==='done')o.frequency.exponentialRampToValueAtTime(880,now+.13);if(type==='season')o.frequency.exponentialRampToValueAtTime(660,now+.25);if(type==='achievement')o.frequency.exponentialRampToValueAtTime(1046,now+.44);o.connect(g);o.start(now);o.stop(now+notes[1]);
 }catch(e){}
}
function vibrate(pattern){if(navigator.vibrate)navigator.vibrate(pattern)}
function addSoundSettings(){const box=document.querySelector('#settings .modalBody');if(!box||document.querySelector('#soundToggle'))return;const row=document.createElement('div');row.className='settingsToggle';row.innerHTML=`<div><b>Sons e resposta tátil</b><div class="hint">Efeitos originais nas interações. Não utiliza áudio oficial da Marvel.</div></div><input id="soundToggle" type="checkbox" ${P.sound?'checked':''}>`;const save=document.querySelector('#saveSettings');box.insertBefore(row,save);row.querySelector('input').onchange=e=>{P.sound=e.target.checked;saveP();tone('done')};}
function patchEpisodeSection(){
 if(!window.current||!document.querySelector('#modal.open'))return;const total=countFor(current);if(!total)return;
...
const oldOpen=window.openItem;window.openItem=function(id){oldOpen(id);setTimeout(()=>{patchEpisodeSection();addSoundSettings()},30)};
const oldRender=window.render;window.render=function(){oldRender();setTimeout(()=>{addSoundSettings()},30)};
bindSounds();setTimeout(addSoundSettings,300);
window.__MARVEL_PREMIUM__={episodeCounts,sound:()=>P.sound};
})();